...
def update_video_crawl_status(video_id, status):
    """Update the crawl status for a specific video."""
    with app.app.app_context():
        try:
            ...
            logging.info(f"Updated crawl status for video {video_id} to {status}")
        except Exception as e:
            logging.error(f"Error updating crawl status for video {video_id}: {e}")
            app.db.session.rollback()

def reset_expired_video_success_states():
    """Reset success states to onready if they are older than 6 hours."""
    with app.app.app_context():
        try:
            ...
            update_stmt = (
                update(app.TikTokVideosInfo)
                .where(
                    (app.TikTokVideosInfo.crawl_status == "success") &
                    (app.TikTokVideosInfo.crawl_status_updated < six_hours_ago)
                )
                .values(
            ...
            if result.rowcount > 0:
                logging.info(f"Reset {result.rowcount} expired video success states to onready")
        except Exception as e:
            logging.error(f"Error resetting expired video success states: {e}")
            app.db.session.rollback()


async def get_comments(video_id, comment_count):
    # Set status to onprocess when starting to crawl comments
    update_video_crawl_status(video_id, "onprocess")
    ...
    try:
        ...
        # Set status to success after comments crawling is complete
        update_video_crawl_status(video_id, "success")
        logging.info(f"Successfully crawled comments for video {video_id}")
        
    except Exception as e:
        logging.error(f"Error crawling comments for video {video_id}: {e}")
        # Reset to onready on error so it can be retried
        update_video_crawl_status(video_id, "onready")


async def check_and_crawl_comments():
    # Reset expired success states to onready (6 hours)
    reset_expired_video_success_states()
    
    with app.app.app_context():
        app.db.create_all()
        
        # Only get videos with onready status for comment crawling
        videos = (
            app.TikTokVideosInfo.query
            .filter(
                (app.TikTokVideosInfo.crawl_status == "onready") &
                (app.TikTokVideosInfo.video_commentcount > 1)
            )
        ...
        if not videos:
            logging.info("No videos with 'onready' status found for comment crawling")
            return
            
        if len(videos) > 5:
            videos = random.sample(videos, 30)
            
        logging.info(f"Selected {len(videos)} videos for comment crawling")

...
class UserInfo:
    ...
    async def user_profile_data(all_users):
        ...
        # configurable tokens (proxy disabled by default)
        proxy = None
        ms_tokens_env = os.environ.get("MS_TOKENS")
        if ms_tokens_env:
            ms_tokens_list = [t.strip() for t in ms_tokens_env.split(",") if t.strip()]
        else:
            ms_tokens_list = [
                "eKceaaF8LSRg8q2vT8QePTPF8RD9j1Xl7tiekKtqC0kDU5yldprw_wNVHNrHYoDP0vjgsqb89-xYjKh7EhIE6vULnluXTmAfuwSNka8BbUltC7zQ8Cc6thWEFM4bvnYWdees4FY34mJ2Fw=="
            ]

        async with TikTokApi() as api:
            try:
                headless_flag = os.environ.get("HEADLESS", "True").lower() in ("1", "true", "yes")
                browser_choice = os.environ.get("BROWSER", "chromium")
                await api.create_sessions(
                    ms_tokens=ms_tokens_list,
                    num_sessions=1,
                    sleep_after=int(os.environ.get("SLEEP_AFTER", "5")),
                    headless=headless_flag,
                    browser=browser_choice,  # webkit is not supported, using chromium instead
                    context_options={
            ...
            for user_name in all_users:
                # mark per-user processing
                update_crawl_status(user_name, "onprocess")
                UserInfo.source = user_name
                # small randomized initial delay
                await asyncio.sleep(random.uniform(1.0, 3.0))
                success = False
                max_retries = int(os.environ.get("CRAWL_RETRIES", "3"))
                for attempt in range(1, max_retries + 1):
                    try:
                        user = api.user(user_name)
                        user_data = await user.info()

                        if not user_data or get_nested(user_data, "userInfo", "user") is None:
                            logging.warning(f"Invalid or empty user data for '{user_name}' attempt {attempt}: {user_data}")
                            raise EmptyResponseException(user_data, "Missing userInfo.user")

                        UserInfo.o_data = json.loads(json.dumps(user_data))
                        ...
                        try:
                            ...
                            logging.warning(f"Empty videos response for '{user_name}' attempt {attempt}: {e}")
                            raise

                        if not user_videos:
                            logging.info(f"No videos for user '{user_name}'")
                            update_crawl_status(user_name, "onready")
                            success = False
                            break

                        UserInfo.vo_data = json.loads(json.dumps(user_videos))
                        await UserInfo.insert_video(user_name)
                        update_crawl_status(user_name, "success")
                        success = True
                        break
                    except EmptyResponseException as e:
                        logging.debug(f"EmptyResponseException for {user_name} attempt {attempt}: {e}")
                        backoff = min(30, (2 ** attempt) + random.uniform(0.5, 1.5))
                        await asyncio.sleep(backoff)
                    except Exception as e:
                        logging.error(f"Error crawling {user_name} attempt {attempt}: {e}")
                        backoff = min(30, (2 ** attempt) + random.uniform(0.5, 1.5))
                        await asyncio.sleep(backoff)

                if not success:
                    logging.info(f"Marking '{user_name}' as onready after failures")
                    update_crawl_status(user_name, "onready")
...
async def check_and_crawl_videos():
    ...
    try:
        ...
        all_users = session.query(users).with_entities(app.TikTokSources.source_name).filter(
            ...
            (app.TikTokSources.source_check == True) & (app.TikTokSources.owner == 2) &
